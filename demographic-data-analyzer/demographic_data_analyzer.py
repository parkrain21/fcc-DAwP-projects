...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = round(df.loc[df['education'] == 'Bachelors'].value_counts().sum() / len(df) * 100, 1)
    ...
    rich_countries = round(df.loc[df['salary'] == '>50K', 'native-country'].value_counts() / df.loc[:, 'native-country'].value_counts() * 100, 1)
    highest_earning_country = rich_countries.sort_values(ascending=False).index[0]
    highest_earning_country_percentage = rich_countries.sort_values(ascending=False)[0]

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df['occupation'].loc[df['native-country'] == 'India'].value_counts().index[0]
